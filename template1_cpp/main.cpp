...
void processPlayerMovement(Player &player, std::string& chars)
{
  if (Input.keys[GLFW_KEY_W])
    player.ProcessInput(MovementDir::UP, chars, flag_prick, flag_exit);
  else if (Input.keys[GLFW_KEY_S])
    player.ProcessInput(MovementDir::DOWN, chars, flag_prick, flag_exit);
  if (Input.keys[GLFW_KEY_A])
    player.ProcessInput(MovementDir::LEFT, chars, flag_prick, flag_exit);
  else if (Input.keys[GLFW_KEY_D])
    player.ProcessInput(MovementDir::RIGHT, chars, flag_prick, flag_exit);
}
...
int main(int argc, char** argv) {
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        processPlayerMovement(player, chars1);
		restoreBackGround(screenBuffer, floor, player);
		player.DrawMy(screenBuffer, floor, man); // draw or drawMy
